...
class Product extends \yii\db\ActiveRecord
{
    ...
    public function getLimitProduct($limit=4)
    {
        $data = Product::find()->asArray()->limit($limit)->all();
        return $data;
    }

     public function getRandomProduct($limit=9)
    {
        $data = Product::find()->asArray()->orderBy(['rand()' => SORT_DESC])->limit($limit)->all();
        return $data;
    }

    public function getNewProduct($limit=8)
    {
        $data = Product::find()->asArray()->orderBy(['created_at' => SORT_DESC])->limit($limit)->all();
        return $data;
    }
}
